...
Updated for A-n53-k7 Instance

Benchmark Algorithms:
1. Gurobi (Commercial Solver) - Absolute optimality reference (small instances only)
...
4. Gurobi integration for optimality benchmarking (limited scalability)
"""
...
# ============================================================================
# DATA DEFINITION - A-n53-k7 Instance
# ============================================================================
...
class GurobiSolver:
    ...
    NOTE: For A-n53-k7 (34 customers), Gurobi may struggle due to complexity.
    """

    def __init__(self, coords, demands, capacity, threat_zones, max_vehicles=7, time_limit=120):
        self.coords = coords
    ...
    def _build_model(self):
        ...
        model.setParam('MIPGap', 0.05)  # 5% gap for larger instance
...
def run_comprehensive_experiment(n_runs=1):
    ...
    print("=" * 80)
    print("A-n53-k7 Instance - Algorithm Comparison (WITH GUROBI BENCHMARK)")
    print("=" * 80)
    print("\nInstance Statistics:")
    ...
    print("\nAlgorithms:")
    if GUROBI_AVAILABLE:
        print("  0. Gurobi - Commercial MIP Solver (Optimality Reference - Limited Scalability)")
    print("  1. Memetic Framework (Proposed) - Hybrid exploration-exploitation with threat evasion")
    print("  2. ALNS - Adaptive Large Neighborhood Search (Røpke & Pisinger, 2006) - Gold standard")
    print("  3. HGA - Hybrid Genetic Algorithm (Liu et al., 2014) - Baseline")
    ...
    for name, algo_class, kwargs in algorithms:
        ...
        if n_runs > 1 and name != "Gurobi":  # Gurobi is deterministic, single run sufficient
            ...
            for run in range(n_runs):
                ...
                print(f"    Run {run+1}: Cost={result['cost']:.2f}, Distance={result['distance']:.2f}, "
            ...
            if all_results:
                ...
                if costs:
                    stats = {
                        ...
                        'success_rate': len(costs) / n_runs * 100
                    }
            ...
            try:
                ...
                print(f"  Distance: {result['distance']:.2f}, Threat: {result['threat_exposure']}, Valid: {result['valid']}")
            except Exception as e:
    ...
    print_performance_table(
        ...
        title="\n📊 TABLE 1: Performance Comparison (A-n53-k7 Instance)"
    )
    ...
    if GUROBI_AVAILABLE:
        ...
        if gurobi_result and gurobi_result.get('cost', float('inf')) < float('inf'):
            ...
            print("However, for this larger instance (34 customers), Gurobi:")
            print("  • Required significantly more memory (O(n²) variables)")
            print("  • May have hit time limit before proving optimality")
            print("  • Does not scale well beyond this size")
            print("→ Proposed memetic framework is designed for scalability where Gurobi fails.")
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("\nKey Takeaway: For the larger A-n53-k7 instance (34 customers),")
...
COMPREHENSIVE STATISTICAL ANALYSIS - A-n53-k7 Instance
...
def statistical_comparison(algorithms, coords, demands, capacity, threat_zones,
    ...
    print("=" * 80)
    print(f"Instance: A-n53-k7 ({len(coords)-1} customers, {max_vehicles} vehicles)")
    ...
    # ------------------------------------------------------------------------
    print("\n" + "=" * 80)
    print("SUMMARY PERFORMANCE TABLE (Ranked by Mean Cost)")
    print("=" * 80)

    all_stats_sorted = sorted(all_stats, key=lambda x: x.get("mean_cost", float("inf")))
    columns = [
    ...
    print_performance_table(all_stats_sorted, columns)
    ...
    if gurobi_stats and np.isfinite(gurobi_stats['best_cost']):
        ...
        print(f"  Note: Gurobi may have hit time limit without proving optimality")
...
A-n53-k7 Instance (53 customers, 7 vehicles, 5 threat zones)
...
# Threat zones (5 zones scaled appropriately for the coordinate range)
threat_zones = [
...
def run_full_experiment(n_runs=30):
    ...
    print_performance_table(
        results_sorted,
        columns=columns,
        title="\n📊 TABLE 1: Best Valid Solutions Comparison (A-n53-k7)"
    )
    ...
    print_performance_table(
        ...
        title="\n📊 TABLE 2: Statistical Performance Over 30 Independent Runs (A-n53-k7)"
    )
